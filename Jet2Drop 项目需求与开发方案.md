...
浏览中心仓库、普通上传下载和非照片快传依赖 Windows，因此这些功能要求 Windows 开机、未睡眠、Tailscale 在线且 SFTPGo 服务正常。照片快传只有在接收端 Jet2Drop 桌面应用在线且已注册照片接收地址时才走 Tailscale 直连；接收端离线或直连失败时，照片不会自动改走仓库中转，用户重新选择照片后再发送。Tailscale 负责网络连通，Jet2Drop 只绑定 Tailscale 地址，不监听公网地址。
...
13. 设备间快传：照片在目标桌面应用在线时通过 Tailscale 标准 HTTP 直传；其他文件不论大小均通过 Windows SFTP 仓库中转。
14. 非照片快传保留收件箱、有效期、领取进度、完整性校验和清理状态；照片直传完成后直接落到目标桌面端默认保存目录，不进入收件箱。
15. 照片始终按原始字节传输，不压缩、缩放、转码或以预览内容替代原文件；接收时写 `.part`，边接收边计算 SHA-256，校验成功后原子改名，同名自动生成不冲突名称。
...
- 拖入时显示明确的目标目录和投放状态。
- 在设置中选择并记忆桌面端快传接收默认保存目录，可随时修改；快传领取和照片接收不再每次弹出保存位置选择，普通仓库下载仍保留手动选择位置。
- 导出整个仓库到指定本地目录。
- 从指定本地目录导入整个仓库。
- macOS 提供完整仓库的增量双向同步。
...
- 常见图片与小型UTF-8文本预览。
- 浅色/深色主题、自定义背景和按钮样式。
- 通过 Windows 中转的非照片文件快传；接收方可以稍后上线领取，支持暂停、取消、重试、续传和校验。
- 照片使用 Tailscale 标准 HTTP 直传，接收端必须在线；失败后重新选择照片发送，不自动回退 SFTP。照片原图只落到默认保存目录，预览或播放只在传输完成后进行。
...
- 文件历史浏览、搜索、收藏和最近访问。
- 公开分享、多人协作或在线编辑等扩展能力。
- 在有真实稳定性和速度数据后，再评估是否把照片直传协议扩展到其他文件类型；当前不扩展。
...
基础版快传明确分为两条路径：照片使用 Tailscale + 标准 HTTP 直接写入目标桌面端默认保存目录；其他所有文件（不按大小区分）复用 Tailscale + SFTP，由 Windows 暂存。非照片的 Windows 中转流程如下：
...
照片直传只处理明确识别为图片的文件：发送端读取原始文件流，目标桌面端在 Tailscale 地址上运行随应用生命周期启停的标准 HTTP 接收器。接收器先写 `.part`，边接收边计算 SHA-256，校验通过后原子改名；同名文件自动生成不冲突名称。照片不压缩、不转换、不进入仓库或收件箱。目标设备离线、令牌无效或直连失败时，任务直接失败并提示重新选择照片发送，不自动切换到 SFTP。
...
│                ↕ D:\Jet2DropTransfer          │
│  Jet2Drop HTTP 照片接收器（仅 Tailscale 地址） │
│                ↕ 默认保存目录                  │
│        │                                      │
...
照片快传使用一条独立的短时直连通道：发送端和接收端通过 Tailscale 私网地址建立带设备令牌的标准 HTTP 流；接收器跟随桌面应用生命周期运行，只绑定 Tailscale 地址，不监听公网地址。直连不可用时任务直接失败并提示重新发送，不改变非照片文件的可靠 SFTP 中转链路。
...
| 三端快传 | 非照片文件使用已验证的 SFTP 中转；照片使用 Tailscale 地址上的标准 HTTP 直传，失败后重新发送，不回退中转；两条路径各自校验 SHA-256 | 基础版可实现；照片直传要求目标桌面应用在线，普通中转可延迟领取 |
...
- 取消操作关闭当前流并清理临时文件。
- 非照片文件沿用可靠链路的续传；照片直传不做暂停、分片或跨应用重启续传，失败后重新选择发送。
...
照片快传不创建上述投递目录：目标桌面端在 Tailscale 地址上提供 `/v1/photo` HTTP 接口，设备注册资料携带地址和令牌。发送端只在文件被明确识别为图片时调用该接口；其他文件不会因为体积小而进入照片通道。
...
默认有效期为 7 天，可选 1 天或 30 天。首期限制单任务最多 100 个文件、总计 10 GB；限制写成设置项，目的是防止误选整个磁盘，并非协议上限。非照片文件无论大小都沿用现有可靠 SFTP/Windows 中转链路；照片直传使用单个标准 HTTP 流，不按大小切换链路、不做分片。可靠链路保留流式 SHA-256 校验。
...
媒体文件可记录 `mimeType`，但当前照片直传只接受明确的受支持图片扩展名与图片 MIME 的组合。发送端和接收端都按流处理，不把完整文件载入内存；照片直传不压缩、不转换、不并行分片。视频及其他非照片文件仍使用可靠链路，不因体积进入照片通道。
...
桌面端在设置中选择并记忆默认保存目录，可随时修改。任务开始时捕获该目录：目录修改不会影响已经开始的任务，只对新任务生效。目录不存在或不可写时明确报错，不静默换目录。macOS 使用安全作用域书签持久保存用户授予的目录权限，升级到该实现后首次运行需要重新选择一次目录，之后重启应用仍可直接接收。非照片快传仍写入 `.part`、校验大小及 SHA-256，成功后才记录回执；Android 仍按现有 SAF 规则由用户选择保存位置。照片直传不进入收件箱，接收器直接在默认目录写 `.part`，校验通过后原子改名；同名文件自动生成不冲突名称。照片失败时清理 `.part`，用户重新发送。
...
- 非照片可靠链路继续支持从临时文件续传；不为照片直传增加跨重启续传状态。
- 非照片下载在高延迟中继链路下使用有界 SFTP 流水读取并复用现有连接，避免 16 KiB 小块往返和领取前强制重连；流水窗口不改变 `.part`、断点续传、暂停取消、大小与 SHA-256 校验或原子发布语义。
...
```text
照片任务
→ 目标桌面应用在线并在设备注册中提供 Tailscale 地址和令牌
→ 发送端复用标准 HTTP keep-alive 连接发送原始字节
→ 目标端在默认保存目录写入唯一 `.part` 文件
→ 目标端边接收边计算 SHA-256，大小和哈希一致后原子改名
→ 返回已保存的最终文件名
```

实现约束：

- 接收服务跟随 Windows/macOS Flutter 应用生命周期启动和停止，只绑定检测到的 Tailscale IPv4 地址，不监听公网地址。
- 设备注册资料携带接收地址和随机设备令牌；发送端只有在文件被明确识别为图片时才调用照片接口。
- 快传刷新只由后台控制器统一调度：停留在快传页时每 15 秒刷新一次任务和设备，其他页面每 45 秒维护一次；设备在线资料最多每 45 秒发布一次，不由页面额外启动计时器。
- 文件名经过跨平台净化；同名文件在写入前生成 `name (2).ext` 等不冲突名称，并通过串行名称预留避免并发覆盖。
- 接收失败只清理当前 `.part` 文件并返回明确错误；不写入仓库、不进入收件箱、不暂停、不续传、不离线排队，也不自动回退 SFTP。用户重新选择照片发送。
- 不增加并行分片、按大小切换、复杂自定义协议、独立守护进程、推送系统或多路兜底。普通文件（包括所有大小的非照片文件）继续使用已验证的 SFTP/Windows 中转链路。
...
Android 工程设置 `minSdk = 24`，目标/编译 SDK 使用 Flutter 3.44.9 模板支持的 API 36 或更新的兼容值。普通仓库访问和照片直传发送只需要网络权限；当前照片直传接收服务仅运行在桌面端，不为 Android 增加常驻监听或额外前台服务：
...
20. Windows 与 Android 分别测试照片直传；确认 Windows 快传目录没有照片副本，失败时不会生成中转任务，`.part` 会清理。
21. 在相同网络和设备条件下验证照片单流传输的连接建立、原始字节、SHA-256、同名改名和默认目录行为；只有出现实际瓶颈时才评估额外优化。
...
- 照片直连无法稳定建立、校验不一致、失败残留正式文件，或默认目录无效时静默换目录。
...
- 照片直传 HTTP 接收器、默认保存目录、边接收边校验和失败清理。
- Android SAF保存。
- 桌面拖拽。
...
- 传输任务面板。
- 快传发送、收件箱、领取和结果状态。
- 照片直连的基础进度和结果提示；不增加多路链路选择或复杂速度模式。
- 浅色/深色主题。
...
- 照片直连失败清理、默认目录权限、同名处理和桌面应用退出后的任务边界。
...
10. Windows重启后服务自动恢复。
11. 安装和使用不依赖付费云盘。
12. 非照片快传支持接收端稍后上线领取；照片直传要求目标桌面应用当时在线。
13. 快传文件不进入正式仓库，发布前不显示，领取后通过 SHA-256 校验。
14. Windows 与另一设备不在同一局域网时，仓库浏览、传输和快传仍可使用。
15. 快传不改变文件大小或格式；非照片可靠链路支持网络中断后的续传，照片直传失败后重新发送，任何路径都不能产生损坏的正式文件。
16. 双方在线且目标桌面应用已注册时，照片绕过 Windows 直传并校验后写入默认目录；失败时明确提示重新发送，不自动转入中转队列。非照片文件继续使用 Windows 中转。
...
- 进一步的传输诊断和性能优化；当前不新增媒体分片或按大小切换链路。
- 自动后台同步和计划任务。
- PDF首屏预览。
- HEIC/HEIF统一预览和缩略图。
- 文件收藏和最近访问。
