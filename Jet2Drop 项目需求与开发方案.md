...
桌面端在设置中选择并记忆默认保存目录，可随时修改。任务开始时捕获该目录：目录修改不会影响已经开始的任务，只对新任务生效。目录不存在或不可写时明确报错，不静默换目录。非照片快传仍写入 `.part`、校验大小及 SHA-256，成功后才记录回执；Android 仍按现有 SAF 规则由用户选择保存位置。照片直传不进入收件箱，接收器直接在默认目录写 `.part`，校验通过后原子改名；同名文件自动生成不冲突名称。照片失败时清理 `.part`，用户重新发送。
